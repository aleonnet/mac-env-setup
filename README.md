...
Em terminal interativo, abre o **seletor de perfis** — com "Repetir última instalação" quando houver seleção salva e ajuste **item a item com busca** no modo Personalizado (digite para filtrar por nome/descrição, `Tab` marca, `Enter` confirma). Sem TTY (CI etc.), usa o perfil `terminal`. Cada execução salva a seleção em `~/.config/macenv/state` e o relatório com tempos em `~/.config/macenv/last-run.log`.
...
- **dev** — git (Homebrew), GitHub CLI, jq, wget, Docker Desktop, Node.js + pnpm + bun, pyenv (versões do Python — pacotes por projeto ficam com `python -m venv .venv`), **Claude Code** (instalador nativo da Anthropic)
...
- **`~/.zshrc`** — detecta o terminal: no **Terminal da Apple** mantém visual padrão (prompt clássico, eza sem ícones — sem Nerd Font necessária); nos demais, experiência completa. Montado por blocos conforme a seleção (pyenv, JAVA_HOME, fzf, zoxide, eza, bat, plugins, prompt). Backup do anterior em `~/.zshrc.backup.<timestamp>` antes de sobrescrever — e tudo que outras ferramentas tiverem anexado ao final do arquivo anterior é **migrado automaticamente** para a seção "Suas adições" do novo (com dedupe).
...
- **iTerm2** — fonte Nerd aplicada **sempre que o iTerm2 estiver instalado** (não precisa tê-lo selecionado), porque o `.zshrc` gerado entrega prompt powerline e `eza --icons` a todo terminal que não seja o Terminal da Apple. Vai no perfil padrão **quando o iTerm2 está fechado** — então prefira rodar o instalador de outro terminal (Ghostty, Terminal). Com o iTerm2 aberto (inclusive quando o instalador roda dentro dele) o plist não pode ser escrito, porque o iTerm2 o sobrescreve ao sair; nesse caso é criado o Dynamic Profile **"MacEnv"**, que herda todo o seu perfil Default e só troca a fonte — escolha Profiles → MacEnv, ou feche o iTerm2 e re-execute. O relatório final avisa. Fonte Nerd já configurada é preservada, e o perfil padrão só é editado se ele for de fato o padrão (`Default Bookmark Guid`).
